...
def test_month_timerange(start, end, expected):
    from .partition import month_timerange
    l = month_timerange(start, end)
    assert l == expected


def test_generate_partitioned_table_ddl():
    from .partition import generate_partitioned_table_ddl
    parent_name = 'sale'
...
def test_generate_trigger_condition():
    from .partition import generate_trigger_conditions
    parent_name = 'sale'
    partition_key = 'sold_at'
    month_range = [
        {'start': datetime(2016, 11, 1), 'end': datetime(2016, 12, 1)},
...
def test_generate_trigger():
    from .partition import generate_trigger
    parent_name = 'sale'
    partition_key = 'sold_at'
    month_range = [
        {'start': datetime(2016, 11, 1), 'end': datetime(2016, 12, 1)},
